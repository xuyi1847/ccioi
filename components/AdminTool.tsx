...
const AdminTool: React.FC = () => {
  ...
  return (
    <div className="h-auto lg:h-full min-h-0 flex flex-col gap-3 sm:gap-5">
      ...
      {adminView === 'api' ? <>
        <div className="flex flex-wrap justify-between gap-3 rounded-2xl border border-app-border bg-app-surface/50 p-4"><div><div className="font-bold text-app-text">API Key 与外部任务</div><div className="text-[10px] text-app-subtext mt-1">请求头：X-API-Key · <a href="/video-api-guide.html" target="_blank" rel="noreferrer" className="text-cyan-400 hover:underline">查看中文接入文档</a> · <a href="/api/docs" target="_blank" rel="noreferrer" className="text-cyan-400 hover:underline">Swagger</a></div></div><button onClick={createKey} className="px-3 py-2 rounded-xl bg-cyan-600 text-white text-xs font-bold flex items-center gap-1.5"><Plus size={13}/>创建 API Key</button></div>
