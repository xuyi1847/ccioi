...
const VideoTool: React.FC = () => {
      ...
      try {
        ...
        if (data.type === 'task_finished') {
          if (data.status === 'success' && data.output?.public_url) {
            ...
            notify.success("Sequence rendered successfully.");
            ...
            setIsGenerating(false);
            notify.error("Task failed. Check telemetry log.");
            disconnect();
          }
        }
      } catch (e) {}
  ...
  const handleOptimize = async () => {
    ...
    try {
      ...
      setPrompt(optimized);
      notify.success("Narrative optimized.");
    } catch (error) {
      try {
        const optimized = await clientSideOptimize(prompt, 'VIDEO');
        setPrompt(optimized);
        notify.info("Optimized via edge AI.");
      } catch (fallbackError) { notify.error("Optimization cluster offline."); }
    } finally { setIsOptimizing(false); }
  };

  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (!user) { notify.error("Login required to upload assets."); return; }
    ...
    if (file) {
      ...
      try {
        ...
        setRefImageUrl(url);
        notify.success("Reference asset linked.");
      } catch (err) { notify.error("Cloud storage upload failed."); } 
  ...
  const handleDispatch = async () => {
    ...
    try {
      ...
      notify.error("Dispatcher failed to establish bridge.");
      setIsGenerating(false);
      disconnect();
    }
  };
    ...
    <div className="flex flex-col lg:flex-row h-full gap-6 p-1 overflow-hidden flex-1 tracking-tight">
        ...
        <div className="bg-app-surface/60 p-5 rounded-3xl border border-app-border shadow-xl backdrop-blur-md">
          <div className="flex items-center justify-between mb-5">
            ...
            <div className={`px-2 py-0.5 rounded-full text-[9px] font-bold flex items-center gap-1.5 ${isConnected ? 'bg-green-500/20 text-green-400' : 'bg-app-subtext/20 text-app-subtext'}`}>
              {isConnected ? <Wifi size={10} /> : <WifiOff size={10} />}{isConnecting ? 'Connecting...' : isConnected ? t('socket.connected') : 'Bridge Offline'}
            </div>
          </div>
          <div className="space-y-4">
            ...
            <div><div className="flex items-center justify-between mb-1.5"><label className="block text-[10px] font-bold text-app-subtext uppercase tracking-widest">{t('tool.video.prompt')}</label>
              ...
              </div><div className="relative"><textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} className="w-full bg-app-surface border border-app-border rounded-xl p-2.5 text-app-text text-xs h-20 outline-none focus:border-cyan-500 transition-colors resize-none" />
      ...
      <div className="flex-1 flex flex-col gap-6 min-h-0 h-full overflow-hidden">
        <div className="bg-app-surface/30 rounded-3xl border border-app-border flex-1 flex flex-col items-center justify-center relative overflow-hidden group/preview p-0 bg-black/60 min-h-0">
              ...
              <div className="flex-1 px-6 pb-6 flex flex-col overflow-hidden">
                 <div className="bg-black/60 border border-white/10 rounded-2xl overflow-hidden flex flex-col h-full shadow-2xl">
                    <div className="bg-white/5 px-4 py-2 flex items-center justify-between border-b border-white/5 shrink-0">
                       <div className="flex items-center gap-2 text-[9px] font-bold text-cyan-400 uppercase tracking-widest"><Terminal size={10} /> Live Logs</div>
                    ...
                    {showConsole && <div className="flex-1 overflow-y-auto p-4 font-mono text-[9px] leading-relaxed custom-scrollbar bg-black/40">
                      {logs.length === 0 ? <div className="text-app-subtext/40 animate-pulse">Waiting for telemetry...</div> : logs.map((log, i) => (
                        <div key={i} className={`mb-1 break-all ${log.stream === 'stderr' ? 'text-rose-400' : 'text-cyan-100/70'}`}><span className="opacity-20 mr-2">{new Date().toLocaleTimeString([], { hour12: false, minute:'2-digit', second:'2-digit' })}</span>{log.line}</div>
                      ))}
                      <div ref={logEndRef} />
                    </div>}
                 </div>
          ...
          {generatedVideoUrl ? <div className="w-full h-full flex items-center justify-center animate-fade-in group relative bg-black overflow-hidden"><video src={generatedVideoUrl} className="w-full h-full object-contain z-10" controls autoPlay loop playsInline />
            <div className="absolute top-6 right-6 opacity-0 group-hover:opacity-100 transition-opacity flex gap-3 z-30"><a href={generatedVideoUrl} download target="_blank" rel="noopener noreferrer" className="p-2.5 bg-black/80 hover:bg-app-accent rounded-full text-white shadow-xl"><Download size={20} /></a><button onClick={() => { setGeneratedVideoUrl(null); disconnect(); }} className="p-2.5 bg-red-600/80 hover:bg-red-500 rounded-full text-white shadow-xl"><Trash2 size={20} /></button></div>
          </div> : !isGenerating && !isConnecting && <div className="max-w-md space-y-8 animate-fade-in text-center p-8"><div className="w-20 h-20 bg-cyan-500/10 rounded-full flex items-center justify-center mx-auto relative"><Video size={40} className="text-cyan-400" /></div><div className="space-y-4"><h3 className="text-2xl font-bold text-app-text tracking-tight">Task Dispatcher</h3><p className="text-app-subtext text-xs leading-relaxed">Ready for high-fidelity production. WS established per session.</p></div><div className="flex items-center justify-center gap-2 text-[9px] text-app-subtext font-bold uppercase tracking-[0.2em] bg-app-surface/40 py-2.5 px-6 rounded-full border border-app-border">Bridge Standby</div></div>}
        </div>
      </div>
    </div>
  );
