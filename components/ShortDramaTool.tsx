...
type Engine = 'ccioi-speed' | 'ccioi-standard';
...
const sampleProject = (): Project => {
  ...
  return {
    ...
    data: {
      ...
      shots: specs.map(([title, scene, camera, dialogue, prompt, character_ids, continuity_from_previous], index) => ({ id: uid(), title, scene, shot_size: index % 4 === 0 ? '全景' : '中近景', camera, duration: 6, engine: 'ccioi-speed', status: 'draft', seed: 7301 + index, character_ids, dialogue, prompt, continuity_from_previous })),
    },
  };
};
...
const shortSampleProject = (kind: SampleKind): Project => {
  ...
  return { name: selected.name, data: { synopsis: selected.synopsis, style: selected.style, aspect_ratio: '16:9', characters: selected.characters, script: selected.script, audio_assets: [], shots: selected.specs.map(([title, scene, camera, dialogue, prompt, character_ids, continuity_from_previous], index) => ({ id: uid(), title, scene, shot_size: index === 0 || index === 4 ? '全景' : '中近景', camera, duration: 6, engine: 'ccioi-speed', status: 'draft', seed: 8601 + index, character_ids, dialogue, prompt, continuity_from_previous })) } };
};
...
const ShortDramaTool: React.FC = () => {
  ...
  const loadProjects = async () => {
    ...
    try {
      ...
      if (items.length) { setProjects(items); setProject(items[0]); }
        ...
        notify.success('已创建 30 秒 CCIOI 科幻样片');
      }
  ...
  const dispatchShot = async (shot: Shot) => {
    ...
    sendCommand({ type: 'TASK_EXECUTION', task: 'VIDEO_GENERATION', token: user.token, model: shot.engine,
      parameters: { model: shot.engine, prompt: generationPrompt, dialogue: shot.dialogue, generate_audio: shot.engine === 'ccioi-speed', width: portrait ? 1024 : 1536, height: portrait ? 1536 : 1024, frames: Math.min(481, Math.max(49, Math.round(shot.duration * 24) + 1)), num_frames: Math.min(481, Math.max(49, Math.round(shot.duration * 24) + 1)), fps: 24, seed: shot.seed, image_url: continuityImage, image_frame: continuityImage ? 0 : undefined, image_strength: continuityImage ? (usesPreviousFrame ? 0.78 : 0.9) : undefined, ref_image: shot.engine === 'ccioi-standard' ? continuityImage : undefined, config: 'configs/diffusion/inference/768px.py', steps: 40, ratio: projectRef.current.data.aspect_ratio, motion_score: 6, nproc_per_node: 1, project_id: projectRef.current.id, shot_id: shot.id }
    });
  };
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (message.type === 'TASK_LOG') {
        const match = String(message.line || '').match(/(?:progress|进度)\D*(\d{1,3})%?/i);
        if (match) patchShot(activeShotId, { progress: Math.min(100, Number(match[1])) });
        return;
      }
      ...
      const handleFinished = async () => {
        ...
        patchShot(finishedShotId, success ? { status: 'done', output_url: message.output.public_url, ending_frame_url: endingFrameUrl, preview_url: undefined, progress: 100 } : { status: 'failed' });
  ...
  return <div className="h-auto lg:h-full min-h-0 flex flex-col lg:flex-row gap-3 lg:gap-4 pb-3 lg:pb-0">
    <aside className="w-full lg:w-56 shrink-0 rounded-xl lg:rounded-2xl border border-app-border bg-app-surface/60 p-2.5 lg:p-3 flex flex-col min-h-0">
      ...
      <div className="flex lg:block gap-2 flex-1 overflow-x-auto lg:overflow-x-hidden lg:overflow-y-auto custom-scrollbar mt-2 lg:mt-3 lg:space-y-2 pb-1 lg:pb-0">{projects.map((item) => <button key={item.id} onClick={() => setProject(item)} className={`w-44 lg:w-full shrink-0 text-left p-2.5 lg:p-3 rounded-xl border ${project?.id === item.id ? 'border-cyan-400 bg-cyan-500/10' : 'border-app-border hover:bg-app-surface-hover'}`}><div className="text-xs font-bold truncate">{item.name}</div><div className="text-[9px] text-app-subtext mt-1">{item.data.shots?.length || 0} 个镜头</div></button>)}</div>
    </aside>
    <section className="flex-1 min-w-0 min-h-[68dvh] lg:min-h-0 flex flex-col rounded-xl lg:rounded-2xl border border-app-border bg-app-surface/40 overflow-hidden">
      {!project ? <div className="h-full flex flex-col items-center justify-center text-app-subtext"><Clapperboard size={42} className="opacity-20 mb-4" /><p className="text-sm">新建一个短剧项目开始制作</p></div> : <>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar p-3 lg:p-4">
          ...
          {tab === 'shots' && <div><div className="flex justify-between mb-4"><div><div className="text-xs text-app-subtext">{project.data.shots.length} 个镜头 · 队列 {queue.length}</div><div className="text-[10px] text-emerald-400 mt-1">连续生成：自动用上一镜尾帧承接下一镜</div></div><button onClick={runBatch} disabled={!!activeShotId} className="px-4 py-2 rounded-lg bg-cyan-600 text-white text-xs font-bold disabled:opacity-40">批量生成待处理镜头</button></div><div className="space-y-3">{project.data.shots.map((shot,index)=><div key={shot.id} className="rounded-xl border border-app-border bg-app-base/60 p-3 grid lg:grid-cols-[80px_1fr_150px] gap-3"><div className="text-xs font-bold text-cyan-400">#{index+1}<div className="text-[9px] text-app-subtext mt-1">{shot.duration}s</div><div className="text-[9px] mt-2">{shot.status}</div>{shot.ending_frame_url && <div className="text-[9px] text-emerald-400 mt-2">尾帧已就绪</div>}</div><div><input value={shot.title} onChange={(e)=>patchShot(shot.id,{title:e.target.value})} className="w-full bg-transparent font-bold outline-none"/><textarea value={shot.prompt} onChange={(e)=>patchShot(shot.id,{prompt:e.target.value})} className="mt-2 w-full h-20 bg-app-surface border border-app-border rounded-lg p-2 text-xs"/><input value={shot.dialogue} onChange={(e)=>patchShot(shot.id,{dialogue:e.target.value})} placeholder="对白/字幕" className="mt-2 w-full bg-app-surface border border-app-border rounded-lg p-2 text-xs"/></div><div><select value={shot.engine} onChange={(e)=>patchShot(shot.id,{engine:e.target.value as Engine})} className="w-full bg-app-surface border border-app-border rounded-lg p-2 text-xs"><option value="ccioi-speed">CCIOI Speed</option><option value="ccioi-standard">CCIOI Standard</option></select>{shot.output_url?<video src={shot.output_url} poster={shot.ending_frame_url} controls preload="metadata" className="mt-2 w-full aspect-video bg-black rounded-lg"/>:shot.preview_url?<div className="mt-2 relative aspect-video bg-black rounded-lg overflow-hidden"><img src={shot.preview_url} className="w-full h-full object-contain"/><div className="absolute inset-x-0 bottom-0 bg-black/70 p-1 text-center text-[9px] text-cyan-300">生成预览 {shot.progress ? `${shot.progress}%` : ''}</div></div>:shot.status==='generating'?<div className="mt-2 aspect-video rounded-lg bg-black/70 flex flex-col items-center justify-center text-[9px] text-cyan-300"><Loader2 size={18} className="animate-spin mb-2"/>正在生成 {shot.progress ? `${shot.progress}%` : ''}</div>:<button onClick={()=>dispatchShot(shot)} disabled={!!activeShotId} className="mt-2 w-full py-2 rounded-lg border border-cyan-500/30 text-cyan-400 text-xs">生成镜头</button>}</div></div>)}</div></div>}
          {tab === 'timeline' && <div><div className="rounded-xl border border-app-border bg-black/30 p-4"><div className="text-[10px] text-app-subtext mb-3">VIDEO TRACK</div><div className="flex gap-2 overflow-x-auto pb-3">{project.data.shots.map((shot,index)=><div key={shot.id} className="w-40 shrink-0"><div className="aspect-video bg-black rounded-lg overflow-hidden">{shot.output_url?<video src={shot.output_url} poster={shot.ending_frame_url} preload="none" className="w-full h-full object-contain"/>:shot.preview_url?<img src={shot.preview_url} className="w-full h-full object-contain"/>:<div className="h-full flex items-center justify-center text-[10px] text-app-subtext">{shot.status==='generating'?`生成中 ${shot.progress || ''}${shot.progress ? '%' : ''}`:'待生成'}</div>}</div><div className="text-[10px] mt-1 truncate">#{index+1} {shot.title}</div></div>)}</div><div className="border-t border-app-border pt-3 text-[10px] text-app-subtext">DIALOGUE / TTS TRACK（已保留 voice_id、对白和音频素材结构）</div><div className="mt-2 flex gap-2 overflow-x-auto">{project.data.shots.map((shot)=><div key={shot.id} className="w-40 shrink-0 h-10 rounded bg-emerald-500/10 border border-emerald-500/20 p-2 text-[9px] truncate">{shot.dialogue || '无对白'}</div>)}</div></div><div className="mt-4 flex justify-end"><button onClick={exportFilm} disabled={exporting} className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-fuchsia-600 to-cyan-600 text-white text-xs font-bold flex items-center gap-2 disabled:opacity-50">{exporting?<Loader2 size={14} className="animate-spin"/>:<Film size={14}/>}导出 H.264 / AAC 成片</button></div>{project.data.export_url && <div className="mt-4 rounded-xl border border-green-500/20 bg-green-500/5 p-4"><video src={project.data.export_url} controls preload="metadata" className="w-full max-h-96 bg-black rounded-lg"/><a href={project.data.export_url} download className="mt-3 inline-flex items-center gap-2 text-xs font-bold text-green-400"><Download size={14}/>下载最终成片</a></div>}</div>}
        </div>
      </>}
    </section>
  </div>;
