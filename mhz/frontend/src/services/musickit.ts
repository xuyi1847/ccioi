...
export const musicKit={
  ...
  play:async(trackId:string)=>{if(!instance)throw new Error("MusicKit is not configured");await timeout(instance.setQueue({song:trackId}),"Apple Music 设置播放队列超时");await timeout(instance.play(),"Apple Music 开始播放超时")},
  ...
  observeState:(listener:(playing:boolean,ended:boolean)=>void)=>{if(!instance)throw new Error("MusicKit is not configured");const current=instance;const handler=()=>{const value=current.playbackState;const playing=value===2||value==="playing";const ended=value===5||value===10||value==="ended"||value==="completed";const settled=playing||ended||value===0||value===3||value===4||value==="none"||value==="paused"||value==="stopped";if(settled)listener(playing,ended)};current.addEventListener("playbackStateDidChange",handler);return()=>current.removeEventListener("playbackStateDidChange",handler)},
