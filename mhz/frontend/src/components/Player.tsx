...
export function Player({item,playing,progress,onToggle,onFavorite,onSkip,onDislike}:{item:Recommendation;playing:boolean;progress:number;onToggle:()=>void;onFavorite:()=>void;onSkip:()=>void;onDislike:()=>void}){
  ...
  return <div className="flex w-full flex-col items-center">
    ...
    <div className="mt-8 min-h-16 text-center"><h2 className="text-xl font-medium tracking-tight">{track.title}</h2><p className="mt-1 text-sm text-[var(--muted)]">{track.artist}</p></div>
    ...
    <div className="mt-7 flex items-center gap-7">
      ...
      <button aria-label={playing?"Pause":"Play"} onClick={onToggle} className="grid h-14 w-14 place-items-center rounded-full border border-black/25 text-lg transition hover:bg-black hover:text-white">{playing?"Ⅱ":"▶"}</button>
