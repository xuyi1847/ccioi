...
const VOLUME_KEY="mhz-player-volume-v1";
...
export default function Home(){
  ...
  useEffect(()=>{const timer=window.setTimeout(()=>{const saved=Number(localStorage.getItem(VOLUME_KEY));if(Number.isFinite(saved)&&saved>=0&&saved<=1){setVolume(saved);if(saved>0)lastAudibleVolume.current=saved}},0);return()=>window.clearTimeout(timer)},[]);
  useEffect(()=>{if(audio.current)audio.current.volume=volume;if(musicKitReady)musicKit.setVolume(volume)},[volume,musicKitReady]);
  ...
  const changeVolume=(next:number)=>{const value=Math.max(0,Math.min(1,next));setVolume(value);if(value>0)lastAudibleVolume.current=value;localStorage.setItem(VOLUME_KEY,String(value))};
  const toggleMute=()=>changeVolume(volume===0?lastAudibleVolume.current||.8:0);
