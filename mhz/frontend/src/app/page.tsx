...
export default function Home(){
  ...
  useEffect(()=>{void(async()=>{try{if(!localStorage.getItem("ccioi_auth_token")){setAuthenticated(false);return}setAuthenticated(true);const legacyUserId=localStorage.getItem(LEGACY_USER_KEY);if(legacyUserId){await api.claimLegacy(legacyUserId);localStorage.removeItem(LEGACY_USER_KEY)}const [channels,favorites]=await Promise.all([api.channels(),api.favorites()]);setFavoriteIds(new Set(favorites.trackIds));usePlayer.getState().set({channels,channel:channels[0]})}catch(error){setAuthenticated(false);usePlayer.getState().set({error:error instanceof Error?error.message:"MHz 无法启动"})}})()},[]);
  ...
  return <main className="relative flex min-h-[100svh] flex-col items-center px-4 py-5 sm:px-8 sm:py-8"><div className="noise"/><header className="z-10 flex w-full max-w-5xl items-center justify-between border-b border-black/10 pb-4"><div className="display text-2xl font-bold">MHz</div><div className="caps whitespace-nowrap text-[var(--muted)]"><span className="hidden sm:inline">Personal radio · </span>Apple Music</div></header>
    {!authenticated?<section className="z-10 flex flex-1 flex-col items-center justify-center pb-20 text-center"><div className="display text-[clamp(72px,18vw,170px)] leading-none">87.5</div><h1 className="mt-10 text-2xl sm:text-4xl">请先登录 ccioi</h1><p className="mt-4 text-sm text-[var(--muted)]">MHz 使用你的 ccioi 账号保存喜好和收听记录</p><a className="mt-8 rounded-full bg-black px-8 py-3 text-sm text-white" href="/">返回 ccioi 登录</a></section>:!state.connected?<section className="z-10 flex flex-1 flex-col items-center justify-center pb-20 text-center"><div className="display text-[clamp(72px,18vw,170px)] leading-none">87.5</div><p className="caps mt-3 text-[var(--muted)]">Signal found</p><h1 className="mt-12 max-w-lg text-2xl font-normal sm:text-4xl">不是播放你喜欢的歌，<br/>而是找到你的下一首喜欢。</h1><div className="mt-10"><StartListeningButton busy={state.loading} onStart={connect}/></div><p className="mt-4 text-xs text-[var(--muted)]">使用你自己的 Apple Music 订阅授权播放</p></section>:
