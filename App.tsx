...
const AppContent: React.FC = () => {
  ...
  const [demoVideos, setDemoVideos] = useState<Array<{ id: string; url: string; label: string; thumbnail?: string }>>([]);
  ...
  useEffect(() => {
    mockBackend.getShowcase().then((records) => {
      setDemoVideos(records.map((item: any) => ({
        id: String(item.id),
        url: item.video_url || item.url,
        label: item.prompt || 'CCIOI 精选作品',
            ...
            <div className="w-full max-w-6xl px-4 pb-8 md:pb-12">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
                 {demoVideos.map((video) => (
                   <div key={video.id} className="relative group aspect-video bg-black rounded-2xl md:rounded-3xl border border-app-border overflow-hidden shadow-2xl transition-all hover:scale-[1.02] hover:border-app-accent/50">
                      <video src={video.url} poster={video.thumbnail} className="w-full h-full object-contain" muted loop playsInline preload="none" controls />
                 ...
                 {demoVideos.length === 0 && <div className="sm:col-span-2 md:col-span-4 flex flex-col items-center justify-center min-h-48 bg-app-surface/20 border-2 border-dashed border-app-border rounded-2xl md:rounded-3xl text-app-subtext gap-3 p-6 text-center"><Sparkles size={24} className="opacity-30" /><p className="text-xs">管理员可从历史生成记录中选择首页展示视频</p></div>}
              </div>
